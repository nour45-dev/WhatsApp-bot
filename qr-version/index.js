...
const historyStore = new Map(); // chatId -> [{ role, content }]
...
client.on('message', async (message) => {
  try {
    ...
    if (isFallback) {
      const aiAnswer = await aiFallback.tryAnswer(userMessage, readableFullText, getHistory(message.from));
    ...
    pushHistory(message.from, 'user', userMessage);
    pushHistory(message.from, 'assistant', reply);
